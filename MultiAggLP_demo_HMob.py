...
from my_modules.model_lstm import MultiAggLP
...
num_test_snaps = 50  # Number of test snapshots  约7:3划分训练集与测试集 效果不好再改为8:2
...
edge_index_com_list_list_notnr = np.load('com_list_list_mesh/%s_edge_index_com_list_list.npy' % data_name,
                                         allow_pickle=True)
edge_weight_com_list_list_notnr = np.load('com_list_list_mesh/%s_edge_weight_com_list_list.npy' % data_name,
                                          allow_pickle=True)
...
for epoch in range(num_epochs):
    ...
    if save_flag:
        torch.save(model, 'my_pt/MultiAggLP_lstm_binary_%d_.pkl' % epoch)
    # =====================
    # 验证模型
    model.eval()
    # =============
    ...
    print('Val #%d AUC %f %f f1_score %f %f precision %f %f recall %f %f'
          % (epoch, AUC_mean, AUC_std, f1_score_mean, f1_score_std, precision_mean, precision_std, recall_mean,
             recall_std))
    # ==========
    f_input = open('res/%s_MultiAggLP_lstm_binary_rec_.txt' % data_name, 'a+')
    ...
    if (epoch + 1) % 5 == 0:
        # =====================
        # 测试模型
        # model = torch.load('my_pt/MultiAggLP_lstm_binary_53_.pkl')
        model.eval()
        current_time = datetime.datetime.now().time().strftime("%H:%M:%S")
        # =============
        # RMSE_list = []
