...
# ========================================
# Evaluation Metrics
def get_accuracy_recall_precision_f1_score(adj_est, gnd):
    # 将gnd和adj_est转为二分类标签（不合理）
    ground_truth_labels = (gnd > 0).flatten()
    predicted_labels = (adj_est > 0).flatten()

    # 计算真正例、假正例、真负例、假负例数量
    tp = np.sum(np.logical_and(ground_truth_labels == 1, predicted_labels == 1))
    fp = np.sum(np.logical_and(ground_truth_labels == 0, predicted_labels == 1))
    tn = np.sum(np.logical_and(ground_truth_labels == 0, predicted_labels == 0))
    fn = np.sum(np.logical_and(ground_truth_labels == 1, predicted_labels == 0))
...
def rand_proj(num_nodes, hid_dim):  # 这段代码的作用是生成一个随机投影矩阵，并通过Gram-Schmidt正交化方法将其正交化处理，得到一个正交的投影矩阵。这种投影矩阵通常用于降维、特征提取等应用中。
    '''
