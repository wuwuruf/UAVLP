...
for epoch in range(num_epochs):
    ...
    for i in range(num_train_snaps):
        ...
        decoder_loss = get_single_refined_loss(sparse_beta, gnd_tnr, adj_est)
    ...
    print('Val #%d AUC %f %f f1_score %f %f'
          % (epoch, AUC_mean, AUC_std, f1_score_mean, f1_score))
    # ==========
    f_input = open('res/%s_Node2Vec_binary_rec.txt' % data_name, 'a+')
    f_input.write('Val #%d AUC %f %f f1_score %f %f Time %s\n'
                  % (epoch, AUC_mean, AUC_std, f1_score_mean, f1_score, current_time))
    f_input.close()
    # =====================================
    #     adj_est *= max_thres
...
f1_score_std = np.std(f1_score_list, ddof=1)
# ==============
print('Test AUC %f %f f1_score %f %f'
      % (AUC_mean, AUC_std, f1_score_mean, f1_score))
# ==========
f_input = open('res/%s_Node2Vec_binary_rec.txt' % data_name, 'a+')
f_input.write('Test AUC %f %f f1_score %f %f Time %s\n'
              % (AUC_mean, AUC_std, f1_score_mean, f1_score, current_time))
f_input.close()
# =====================================
#     adj_est *= max_thres
#     for r in range(num_nodes):
