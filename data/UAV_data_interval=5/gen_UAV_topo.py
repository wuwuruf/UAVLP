...
# ========== 构建edge_seq===============
# 读取csv文件
df = pd.read_csv('%s_data.csv' % data_name)
...
np.save('%s_r=%d_edge_seq.npy' % (data_name, r), np.array(edge_seq, dtype=object))
# ====================================================
...
# 保存feat数组为npy文件
np.save('%s_r=%d_feat.npy' % (data_name, r), feat)
# ====================================================
...
# # ======构建模块化矩阵==========这段代码没问题
# # 从npy文件中读取edge_seq
# edge_seq = np.load('%s_r=%d_edge_seq.npy' % (data_name, r), allow_pickle=True)
#
# n = 100
# # max_thres = 100
# num_snaps = 361
...
#
# # 将mod保存到npy文件中
# np.save('%s_r=%d_mod.npy' % (data_name, r), mod)
# # ====================================================
...
# 显示网格线
plt.grid(True)
plt.savefig(f'edge_num_%s_r=%d.png' % (data_name, r))
