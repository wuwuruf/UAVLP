...
mod_seq = np.load('data/UAV_data/%s_mod.npy' % (data_name), allow_pickle=True)  # 模块化矩阵序列 shape为（720，100，100），包含720个矩阵
# ==========
# Get the node features
feat = np.load('data/UAV_data/%s_feat.npy' % (data_name),
               allow_pickle=True)  # （这里的特征序列已经按照节点出现顺序排列好了，故P直接拼），shape为（100，32），特征为32位IP地址
...
for epoch in range(num_pre_epochs):  # 在每个epoch内都进行了训练、验证、测试？？？
    ...
    print('Epoch#%d Pre-Train G-Loss %f' % (epoch, gen_loss_mean))
...
MR_mean = np.mean(MR_list)
MR_std = np.std(MR_list, ddof=1)
print('Test Pre RMSE %f %f MAE %f %f MLSD %f %f MR %f %f\n'
      % (RMSE_mean, RMSE_std, MAE_mean, MAE_std, MLSD_mean, MLSD_std, MR_mean, MR_std))
# ==========
f_input = open('res/%s_IDEA_rec.txt' % (data_name), 'a+')
f_input.write('Test Pre RMSE %f %f MAE %f %f MLSD %f %f MR %f %f Time %s\n'
