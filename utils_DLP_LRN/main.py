...
from utils_DLP_LRN.preprocessing import mask_test_edges
from utils_DLP_LRN.utils import read_data, sample_neg, links2subgraphs, gen_features, generate_node2vec_embeddings, \
    node_label, EarlyStopping
from numpy import *
...
"""
快照总数为180，然而该模型每次只能预测一张快照，需要从待预测时刻快照的70%的边(正样本边和负样本边)中现学；
由于我们的原始设置是最后20张快照用于测试，因此需要跑20次这个代码，每次的快照范围为[150, 159]->160; [151, 160]->161; ... ; [169, 178]->179；
需要的快照范围为[150, 179]，待预测快照范围为[160, 179]
"""
node_num = 100  # Number of nodes in the dynamic network 动态网络中节点总数
...
edge_seq_list = edge_seq_list[150:180]
...
for i in range(num_snaps):
    adj = get_adj_wei(edge_seq_list[i], node_num, max_thres)
    feat_list.append(np.concatenate([feat, adj], axis=1))

# 先设定待预测时刻，后续再设置循环
time_to_pre = win_size

# 从目标测试集合进行提取节点对
# cur_adj8 = read_data(name_pre, 8, node_num, max_thres)
cur_adj8 = get_adj_no_wei(edge_seq_list[time_to_pre], node_num)
...
for i in range(win_size):
    # print("------------------------------------")
    cur_step = time_to_pre - win_size + i  # [待预测时刻前win_size张图(按序)]
    adj.append(get_adj_no_wei(edge_seq_list[cur_step], node_num))
    ...
    node_information = feat_list[cur_step]
...
feat_all = np.zeros(132)
for i in range(win_size):
    cur_step = time_to_pre - win_size + i
    cur_adj = get_adj_no_wei(edge_seq_list[cur_step], node_num)
    adj_all += cur_adj
    feat_all += feat_list[cur_step]
...
def train():
    ...
    for i in range(0, len_train - 10, 10):
        ...
        for c in range(0, 10):
            ...
            for j in range(0, 7):
                ...
                for t in range(0, len(gnnGraph_A_train[j][i + c].edge_pairs), 2):
                    ...
                    curAdj[1][t // 2] = int(
                        gnnGraph_A_train[j][i + c].edge_pairs[t + 1])  # Index of the target node 目的节点索引
            ...
            for t in range(0, len(gnnGraph_A_train[7][i + c].edge_pairs), 2):
                ...
                curAdj_all[1][t // 2] = int(
                    gnnGraph_A_train[7][i + c].edge_pairs[t + 1])  # Index of the target node 目的节点索引
        ...
        loss = BCEWithLogitsLoss()(logits_batch[1:].reshape(1, 10),
                                   torch.tensor([train_labels[i:i + 10]], dtype=torch.float))
        loss.backward()
        optimizer.step()
...
def test(dataloader, labels_true):
    ...
    return roc_auc_score(torch.cat(y_true), torch.cat(y_pred)), average_precision_score(torch.cat(y_true), torch.cat(
        y_pred)), mean_absolute_error(torch.cat(y_true), torch.cat(y_pred)), total_loss / len_test
