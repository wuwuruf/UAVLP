...
for epoch in range(num_epochs):
    ...
    if save_flag:
        torch.save(model, 'my_pt/MultiAggLP_lstm_binary_%d.pkl' % epoch)
    # =====================
    # 验证模型
    model.eval()
    # =============
    ...
    print('Val #%d AUC %f %f f1_score %f %f precision %f %f recall %f %f'
          % (epoch, AUC_mean, AUC_std, f1_score_mean, f1_score_std, precision_mean, precision_std, recall_mean,
             recall_std))
    # ==========
    f_input = open('res/%s_MultiAggLP_lstm_binary_rec.txt' % data_name, 'a+')
    ...
    if (epoch + 1) % 5 == 0:
        # =====================
        # 测试模型
        # model = torch.load('my_pt/MultiAggLP_lstm_binary_70.pkl')
        model.eval()
        current_time = datetime.datetime.now().time().strftime("%H:%M:%S")
        # =============
        # RMSE_list = []
