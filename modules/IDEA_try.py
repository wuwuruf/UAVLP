...
class GenNet_tanh(nn.Module):
    ...
    def __init__(self, FEM_dims, EDM_dims, EAM_dims, dropout_rate, win_size, n_heads):
        ...
        for l in range(self.num_FEM_layers):  # num_FEM_layers是ARE的层数
        ...
        for l in range(self.num_EDM_layers):
            ...
            # 注意力层，需要整个win_size的信息
            self.EDM_layers.append(TemporalAttentionLayer(input_dim=self.EDM_dims[l + 1],
                                                          dropout_rate=self.dropout_rate, num_time_steps=self.win_size,
    ...
    def forward(self, sup_list, feat_list, noise_list, align_list, num_nodes_list, lambd, pred_flag=True):
        ...
        # ==========
        # Feature Extraction Module (FEM)
        FEM_input_list = feat_list  # [win_size+1, num_nodes, feat_dim]
        ...
        for t in range(win_size):  # 构造EDM的输入（Z与R拼接）
            EDM_input = FEM_output_list[t]
            noise = noise_list[t]
            EDM_input = torch.cat((EDM_input, noise), dim=1)  # Concatenate GNN outputs with noise inputs
            EDM_input_list.append(EDM_input)  # EDM_input_list的最终形状：[win_size, num_nodes, FEM_output_dim + noise_dim]
        for l in range(0, self.num_EDM_layers * 3, 3):  # 一层EDM_layers里包含3小层
            # ==========
            GNN_layer = self.EDM_layers[l]
            GNN_output_list = []
            for t in range(win_size):  # win_size个图快照逐个输入GNN
                ...
                GNN_output_list.append(GNN_output)  # [win_size, num_nodes, GNN_output_dim]
            # ==========
            GNN_output_tnr = torch.stack(GNN_output_list).permute(1, 0, 2)
            TA_Layer = self.EDM_layers[l + 1]
            TA_output = TA_Layer(GNN_output_tnr)
            TA_output = TA_output.permute(1, 0, 2)
            EDM_input_list = [matrix for matrix in torch.unbind(TA_output, dim=0)]
        EDM_output_list = EDM_input_list
        ...
        #     pre_state = torch.zeros(num_nodes_list[0], hid_dim).to(device)  # Previous state of (inductive) GRU  全0
        #     RNN_output_list = []
        #     align_output_list = []
        #     for t in range(win_size):
        #         RNN_input = GNN_output_list[t]
        #         RNN_output = RNN_layer(pre_state,
        #                                RNN_input)  # 对齐操作的目的是使得pre_state的行数与RNN_input的行数匹配，即使得上一时间步的隐藏状态的行数与下一时间步的节点数匹配
        #         pre_state = align_unit(align_list[t], FEM_output_list[t], FEM_output_list[t + 1], RNN_output,
        #                                lambd=lambd)  # 对齐
        ...
        # ====================
            ...
            for t in range(win_size):
                ...
                for l in range(self.num_EAM_layers):
                    ...
                    emb_output = torch.tanh(emb_output)
                    emb_input = emb_output
                emb = emb_output  # [num_nodes, EAM_dims[-1]]
                ...
                emb_src = torch.reshape(emb, (1, num_nodes, self.EAM_dims[-1]))  # 在维度0复制num_nodes份
                emb_dst = torch.reshape(emb, (num_nodes, 1, self.EAM_dims[-1]))  # 在维度1复制num_nodes份
                # 利用广播机制，求两两嵌入之间的距离。sum前维度为[num_nodes, num_nodes, self.EAM_dims[-1]]，sum后维度为[num_nodes, num_nodes]
                adj_est = -torch.sum((emb_src - emb_dst) ** 2, dim=2)
                adj_est = 1 + torch.tanh(torch.mul(adj_est, scal))
                # ==========
...
class DiscNet(nn.Module):
    ...
    def __init__(self, FEM_dims, GNN_dims, dropout_rate):
        ...
        for l in range(self.num_GNN_layers):
            self.GNN_layers.append(GraphNeuralNetworkDense(self.GNN_dims[l], self.GNN_dims[l + 1],
                                                           dropout_rate=self.dropout_rate))
        # ==========
        self.output_layer = nn.Linear(in_features=self.GNN_dims[-1], out_features=1)  # 注意这里的输出维度是1（实际模型中是输出num_nodes个1）
        self.dropout_layer = nn.Dropout(p=self.dropout_rate)
    ...
    def forward(self, real_sup_list, real_feat_list, fake_sup_list, fake_feat_list):
        ...
        for i in range(win_size):
            ...
            # ==========
            # Output layer
            real_output = self.output_layer(real_input)  # [num_nodes, 1]
            real_output = self.dropout_layer(real_output)
            real_output = torch.sigmoid(real_output)
            fake_output = self.output_layer(fake_input)  # [num_nodes, 1]
            fake_output = self.dropout_layer(fake_output)
            fake_output = torch.sigmoid(fake_output)
            real_output_list.append(real_output)  # [win_size, num_nodes, 1]
