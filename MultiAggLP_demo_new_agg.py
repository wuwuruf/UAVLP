...
from my_modules.model_new_agg import MultiAggLP
...
# =================
edge_seq_list = np.load('data/UAV_data_interval=5/%s_edge_seq.npy' % data_name, allow_pickle=True)
...
edge_index_com_list_list_notnr = np.load('com_list_list_interval=5/%s_edge_index_com_list_list.npy' % data_name,
                                         allow_pickle=True)
edge_weight_com_list_list_notnr = np.load('com_list_list_interval=5/%s_edge_weight_com_list_list.npy' % data_name,
                                          allow_pickle=True)
...
for epoch in range(num_epochs):
    ...
    if save_flag:
        torch.save(model, 'my_pt/MultiAggLP_new_agg_binary_%d_.pkl' % epoch)
    # =====================
    # 验证模型
    model.eval()
    # =============
    ...
    print('Val #%d AUC %f %f f1_score %f %f precision %f %f recall %f %f'
          % (epoch, AUC_mean, AUC_std, f1_score_mean, f1_score_std, precision_mean, precision_std, recall_mean,
             recall_std))
    # ==========
    f_input = open('res/%s_MultiAggLP_new_agg_binary_rec_.txt' % data_name, 'a+')
    ...
    if (epoch + 1) % 5 == 0:
        # =====================
        # 测试模型
        # model = torch.load('my_pt/MultiAggLP_new_agg_binary_53_.pkl')
        model.eval()
        current_time = datetime.datetime.now().time().strftime("%H:%M:%S")
        # =============
        # RMSE_list = []
