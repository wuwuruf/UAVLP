...
class MultiAggLP(Module):
    ...
    def __init__(self, micro_dims, agg_feat_dim, RNN_dims, decoder_dims,
        ...
        # ==================
        # 学习宏观池化表示
        self.macro_pooling_layers = WeiPool(self.micro_dims[-1], self.micro_dims[-1], dropout_ratio=self.dropout_rate)
        # ==================
        # 学习介观池化表示
        self.meso_pooling_layers = WeiPool(self.micro_dims[-1], self.micro_dims[-1], dropout_ratio=self.dropout_rate)
        # ==================
        # 获取注意力融合表示
        ...
        for l in range(self.num_RNN_layers):
            self.RNN_layers.append(nn.LSTMCell(self.RNN_dims[l], self.RNN_dims[l + 1], bias=True))
        self.RNN_drop_layer = nn.Dropout(p=self.dropout_rate)
        # ==================
        # 解码器
    ...
    def forward(self, edge_index_list, edge_weight_list, feat_list, D_com_list_list, partition_dict_list,
        ...
        for l in range(self.num_RNN_layers):
            ...
            for t in range(win_size):
                output_state, output_cell = RNN_layer(input_RNN_list[t], (pre_state, pre_cell))
                output_state, output_cell = self.RNN_drop_layer(output_state), self.RNN_drop_layer(output_cell)
