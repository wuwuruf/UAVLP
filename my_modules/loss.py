...
def get_corss_reg_rep_loss(beta, gnd_list, pred_adj_list, emb_list, theta, lambda_cross_entropy=20, lambda_rep=10,
                           lambda_reg=0.0005):
    """
    获取适应稀疏情况的不加权重构误差，交叉熵损失和1范数正则化
    """
    ...
    for i in range(win_size):
        ...
        # 计算一阶邻居的损失
        pos_rep_loss = torch.sum(
            weight * torch.sum((emb.unsqueeze(0) - emb.unsqueeze(1)) ** 2, dim=-1))
        ...
        neg_rep_loss = torch.sum(
            neg_samples * torch.sum((emb.unsqueeze(0) - emb.unsqueeze(1)) ** 2, dim=-1))

        # 总损失
        rep_loss = lambda_rep * (pos_rep_loss + neg_rep_loss)

        # 计算1范数正则化项
        l1_regularization = lambda_reg * torch.sum(torch.abs(pred_adj))

        # 总损失
        total_loss = decay * (reconstruction_loss + cross_entropy_loss + l1_regularization + rep_loss)
