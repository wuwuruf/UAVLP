...
class MultiAggLP(Module):
    ...
    def __init__(self, micro_dims, pooling_ratio, agg_feat_dim, RNN_dims, decoder_dims,
        ...
        # ==================
        # 学习宏观池化表示
        self.macro_pooling_layers = SimplePool(self.micro_dims[-1], self.pooling_hidden_dim, self.pooling_ratio,
                                               self.n_heads,
                                               dropout_ratio=self.dropout_rate)
        # ==================
        # 学习介观池化表示
        self.meso_pooling_layers = SimplePool(self.micro_dims[-1], self.pooling_hidden_dim, self.pooling_ratio,
                                              self.n_heads,
                                              dropout_ratio=self.dropout_rate)
        # ==================
        # 获取注意力融合表示
        self.agg_layers = AttMultiAgg_new(self.pooling_hidden_dim * 2, self.agg_feat_dim, self.dropout_rate)
        # ==================
        # 学习时序信息  先用两层LSTM试试！！！
